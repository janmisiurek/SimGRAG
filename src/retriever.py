...
def get_vector_store_class(configs):
    if configs.get('vector_store_backend') == 'chroma':
        from src.chroma_vecdb import VectorStore as VS
    else:
        from src.vecdb import VectorStore as VS
    return VS
...
class Retriever:
    ...
    def __init__(self, configs, KG, type_to_nodes=None):
        ...
        self.VectorStore = get_vector_store_class(configs)
        self.timeout = configs['retriever']['timeout']
        self.final_topk = configs['retriever']['final_topk']

        self.node_vector_store = self.VectorStore(
            self.configs['vector_store_names']['node'])
        ...
        self.relation_vector_store = self.VectorStore(
            self.configs['vector_store_names']['relation'])
        num_relations = len(
            set([each for node, adj in KG.items() for each in adj.keys()]))
        ...
        if type_to_nodes:
            ...
            self.type_vector_store = self.VectorStore(
                self.configs['vector_store_names']['type'])
    ...
    def _dfs_all_edges(self, Q, node, visited_edges):
        neighbors_with_degrees = [(neighbor, Q.degree(neighbor))
    ...
    def retrieve(self, query_graph, mode='greedy'):
        ...
        if not nx.is_connected(Q):
            ...
            for component in nx.connected_components(Q):
                subQ_edges = [each for each in query_graph if each[0]
                              in component and each[2] in component]
        ...
        signal.signal(signal.SIGALRM, lambda signum, frame: (
            _ for _ in ()).throw(Exception("retrieval run time out")))
        ...
        try:
            ...
            query_relations = list(set([data['relation'] for u, v, data in Q.edges(
                data=True) if "UNKNOWN" not in data['relation']]))

            # encode the query nodes and relations
            start = time.time()
            if self.use_type_candidates and len(query_nodes) == 0:
                query_texts = query_nodes + \
                    [each.replace("UNKNOWN", "")
                     for each in unknown_nodes] + query_relations
                query_embeddings = self.model.encode(query_texts)
                query_node_vectors = query_embeddings[:len(query_nodes)]
                query_type_vectors = query_embeddings[len(
                    query_nodes):len(query_nodes)+len(unknown_nodes)]
                query_relation_vectors = query_embeddings[len(
                    query_nodes)+len(unknown_nodes):]
            else:
            ...
            def search_node_vectors():
                return {
                    query_node: {each['entity']['name']: math.sqrt(
                        each['distance']) for each in result if each['entity']['name'] in self.KG}
            ...
            def search_relation_vectors():
                similar_relations = {
                    query_relation: {each['entity']['name']: math.sqrt(
                        each['distance']) for each in result}
            ...
            def search_type_vectors():
                ...
                if len(query_type_vectors) > 0:
                    query_type_vector_search_results = self.type_vector_store.search(
                        query_type_vectors, self.type_sim_topk)
            ...
            similar_nodes = {
                **results['node_vectors'], **results['type_vectors']}
            similar_relations = results['relation_vectors']
            vector_search_time = time.time() - start

            # DFS matching order
            root = sorted(similar_nodes.keys(), key=lambda x: (
                len(similar_nodes[x]) if similar_nodes[x] else len(self.KG)))[0]
            if not similar_nodes[root]:
                raise Exception(
                    "No similar nodes found for the root, too many unknown nodes")
            ...
            if mode == 'greedy':
                ...
                for i in range(Q.number_of_edges()-1, 0, -1):
                    ...
                    if similar_relations[Q_relation]:
                        final_best_score[i -
                                         1] += min(similar_relations[Q_relation].values())
                    if similar_nodes[Q_neighbor]:
                        final_best_score[i -
                                         1] += min(similar_nodes[Q_neighbor].values())

            # DFS matching
            def match(cur_Q_idx, node_matching, matched_KG_edges, cur_score, reuse_nodes):
                if cur_Q_idx == Q.number_of_edges():
                    results.add(cur_score, copy.deepcopy(
                        matched_KG_edges), reuse_nodes)
                    return
                        ...
                        for KG_neighbor in self.KG[cur_KG_node][KG_relation]:
                            if similar_nodes[Q_neighbor] is None or KG_neighbor in similar_nodes[Q_neighbor]:
                                ...
                                if Q_neighbor in node_matching:
                                    ...
                                    node_score = 0 if similar_nodes[
                                        Q_neighbor] is None else similar_nodes[Q_neighbor][KG_neighbor]
                                relation_score = 0 if similar_relations[
                                    Q_relation] is None else similar_relations[Q_relation][KG_relation]
                                to_expand.append(
                                    (KG_relation, KG_neighbor, cur_score + node_score + relation_score, next_reuse_nodes))
                ...
                if mode == 'greedy':
                    ...
                    for KG_relation, KG_neighbor, next_score, next_reuse_nodes in to_expand:
                        ...
                        next_matched_KG_edges = matched_KG_edges + \
                            [(cur_KG_node, KG_relation, KG_neighbor)]
                        match(cur_Q_idx + 1, next_node_matching,
                              next_matched_KG_edges, next_score, next_reuse_nodes)
