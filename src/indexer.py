    ...
    def build_index(self, KG, type_to_nodes=None):
        ...
        for batch_start in tqdm(range(0, len(nodes), batch_size)):
            ...
            node_name_to_id = ({node['name']: node['id']
                               for node in batch_embeddings})
            pickle.dump(batch_embeddings, open(
                f"{processed_data_dir}/node_embeddings/{batch_start // batch_size}", "wb"))
            pickle.dump(node_name_to_id, open(
                f"{processed_data_dir}/node_name_to_id/{batch_start // batch_size}", "wb"))
        print("- done")

        # generate relation embeddings
        print("Generating relation embeddings ...")
        relations = list(set([each for node, adj in KG.items()
        ...
        if (not os.path.exists(f"{processed_data_dir}/relation_embeddings")) \
            ...
            relation_name_to_id = (
                {relation['name']: relation['id'] for relation in relation_embeddings})
            pickle.dump(relation_embeddings, open(
                f"{processed_data_dir}/relation_embeddings", "wb"))
            pickle.dump(relation_name_to_id, open(
                f"{processed_data_dir}/relation_name_to_id", "wb"))
        print("- done")
        ...
        if type_to_nodes:
            ...
            if (not os.path.exists(f"{processed_data_dir}/type_embeddings")) \
                ...
                type_name_to_id = ({type['name']: type['id']
                                   for type in type_embeddings})
                pickle.dump(type_embeddings, open(
                    f"{processed_data_dir}/type_embeddings", "wb"))
                pickle.dump(type_name_to_id, open(
                    f"{processed_data_dir}/type_name_to_id", "wb"))
            print("- done")

        # write node embeddings to vector store
        print("Writing node embeddings to vector store ...")
        node_vector_store = self.VectorStore(
        ...
        if node_vector_store.count() != len(nodes):
            ...
            for batch_start in tqdm(range(0, len(nodes), batch_size)):
                batch_embeddings = pickle.load(
                    open(f"{processed_data_dir}/node_embeddings/{batch_start // batch_size}", "rb"))
        ...
        relation_vector_store = self.VectorStore(
        ...
        if relation_vector_store.count() != len(relations):
            ...
            relation_embeddings = pickle.load(
                open(f"{processed_data_dir}/relation_embeddings", "rb"))
            for i in tqdm(range(0, len(relation_embeddings), batch_size)):
                relation_vector_store.insert(
                    data=relation_embeddings[i:i+batch_size])
        print("- done")

        # write type embeddings to vector store
        if type_to_nodes:
            print("Writing type embeddings to vector store ...")
            type_vector_store = self.VectorStore(
            ...
            if type_vector_store.count() != len(types):
                ...
                type_embeddings = pickle.load(
                    open(f"{processed_data_dir}/type_embeddings", "rb"))
                for i in tqdm(range(0, len(type_embeddings), batch_size)):
                    type_vector_store.insert(
                        data=type_embeddings[i:i+batch_size])
            print("- done")
